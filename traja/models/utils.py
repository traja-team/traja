...
def get_lrschedulers(model_type, model, factor=0.1, patience=10 ):
    ...
    """
    if model_type == 'ae' or 'vae':
        encoder_scheduler = ReduceLROnPlateau(model.encoder_optimizer, mode='max', factor=factor, patience=patience, verbose=True)
        decoder_scheduler = ReduceLROnPlateau(model.decoder_optimizer, mode='max', factor=factor, patience=patience, verbose=True)
        latent_scheduler = ReduceLROnPlateau(model.latent_optimizer, mode='max', factor=factor, patience=patience, verbose=True)
        classifier_scheduler = ReduceLROnPlateau(model.classifier_optimizer, mode='max', factor=factor, patience=patience, verbose=True)
