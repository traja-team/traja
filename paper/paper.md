...
One strategy for compressing the representation of trajectories is binning the coordinates to produce an image as show in \autoref{fig:tripgrid_gen}.
...
$$\begin{bmatrix} x' \\ y' \end{bmatrix} =
 ...
 It can be computed with `traja.autocorrelation` and plotted with `traja.plot_autocorrelation`, as in \autoref{fig:autocorrelation}.
...
From the spectrum in \autoref{fig:power-spectrum}, some peaks are observed.
...
Higher peaks indicate higher repetition of a signal.  
 ...
 Peak velocity in a home cage environment is perhaps less interesting than a distribution of velocity observations, as in \autoref{fig:velocity}.
 Additionally, noise can be eliminated from velocity calculations by using a minimal distance moved threshold, as shown in \autoref{Paper-Fortasyn}.
...
## Algorithms and Statistical Models
 ...
 This requires converting the trajectory to an image (ie, trip grid, see [@tripgrid]) and performing principal component analysis on the image in 2D (\autoref{fig:pca}) or 3D (\autoref{fig:3dpca}).
 ...
 It provides an identical view to PCA ..[explain] as shown in \autoref{fig:lda}.
 ...
 *K-Means clustering on the results of \autoref{fig:pca} reveals a high accuracy of classification, with a few errors. Cluster labels are generated by the model.*
...
spatial trajectories [@cox_gaussian_nodate]. 
 ...
 A recent introduction to methods is available at [@Patterson-Statisticalmodelling].
 HMMs have successfully been used to analyze movement of caribou [@Franke], fruit flies[@Holzmann],
 and tuna[@Patterson], among others.
...
This can further be plotted (eg, \autoref{fig:transition_matrix}) with `traja.plot_transition_matrix()`.
...
Clustering actograms is possible with `df.traja.plot_cluster()` and is demonstrated in \autoref{Paper-Fortasyn}.
